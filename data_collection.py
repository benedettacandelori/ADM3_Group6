...
#                                                        data_collection.py                                                        #
#                                                                                                                                  #
#                    library of function useful to download and parse the html pages of the anime from the web                     #
#                                                                                                                                  #
####################################################################################################################################
...
##########################################################################
#                                                                        #
#           functions to download the link of the anime pages            #
#                                                                        #
##########################################################################

def link_retrieval(filename = 'linksfile.txt'):
    '''
...
##########################################################################
#                                                                        #
#    functions to create the folder structure to save the pages in       #
#                                                                        #
##########################################################################
...
##########################################################################
#                                                                        #
#        functions to download the anime html pages from the web         #
#                                                                        #
##########################################################################

def data_crawling(filename = 'linksfile.txt'):
    '''
    this function retrieves every anime page link from the disk,
    and then crawls every anime page from the web automatically
    
    input: name of the file where the links are stored
    output: total number of anime
    '''
    # we count the number of pages to generate the folder structure
    number_of_pages = anime_count(filename)//50 + 1
...
def crawl(filename = 'linksfile.txt', start_line = 1):
    '''
    this function manages the crawling procedure
    and restarts it in case it fails because of the site overloading
    
    input: name of the link file, number of line (of the link file) from which start the crawling
    ...
    while not finished:
        ...
        start_line, finished = article_downloading(filename, start_line)
...
def article_downloading(filename = 'linksfile.txt', start_line = 1):
    '''
    ...
    input: name of the link file, number of line (of the link file) from which start the crawling
    output: id number of the last crawled anime and a boolean which indicates whether the anime was the last one
    '''
...
##########################################################################
#                                                                        #
#        functions to parse the desired information from the html        #
#        and save the to the disk                                        #
#                                                                        #
##########################################################################

def tsv_gen(filename = 'linksfile.txt', verbose = False):
    '''
    this function creates the folder structure and the tsv file
    if verbose == True when the files are checked for tab and newline characters the results are written on screen
    '''
    ...
    for i in tqdm(range(1,total_anime+1)):
        ...
        # check and correct problems in the file (removes \t characters in the data)
...
def find_aired(aired):
    '''
    here we parse the information about the aired dates in the anime
    '''
...
def createanimecharacters(soup):
    """
    this function is used to retrieve every character and voice actor
    """
    characters = []
...
##########################################################################
#                                                                        #
#     functions to download the anime reviews html pages from the web    #
#     and to parse and write them to the disk                            #
#                                                                        #
##########################################################################


def reviews_crawl(filename = 'linksfile.txt', start_line = 1):
    '''
    this function manages the crawling procedure by restarting
    the crawling in case it fails because of the site overloading
    
    input: name of the link file, number of line anime from which start the crawling
    output: total number of downloaded anime reviews pages
    '''
    ...
    while not finished:
        ...
        #restart the crawl from the last (incorrectly) crawled anime
        start_line, finished = reviews_downloading(filename, start_line)
...
def reviews_downloading(filename = 'linksfile.txt', start_line = 1):
    '''
    this function downloads the page of every anime and saves it in a file named review_i.html
    it stops the crawling if it detects that the site imposed a captcha to be able to access the page
    ...
    with open(filename, 'r') as f:
        ...
        for line in tqdm(f):
            ...
            if (line_num>=start_line):
                ...
                # saves the html of the anime page
                name = 'review_' + str(line_num)+".html"
                with open(name, 'w') as anime:
                    anime.write(str(soup))
...
def txt_gen(filename = 'linksfile.txt'):
    '''
    this function create the folder structure and the txt files that will contain the reviews
    '''
    ...
    #creating the folder structure to save the txt
    folder_generation(total_pages, 'alltxt')
    
    for i in tqdm(range(1,total_anime+1)):
        
        # parse the reviews i in a list
        anime = reviews_parsing(i)
        
        # set txt file name
        page = i//50
        txtname = f'content/alltxt/{page}page/review_{i}.txt'
...
def reviews_parsing(anime_num, max_reviews = 5):
    
    '''
    this function reads the anime_num^th html reviews page and retrieves
    the first 500 characters of the first max_reviews reviews
    
    input: id of anime to analyze, maximum number of reviews to retrieve
    output: list of strings, one per review
    '''
    
    # setting file path
    page = anime_num//50
    path = f'content/allpagess/{page}page/review_{anime_num}.html'
    ...
    with open(path, 'r') as f:
        ...
        # to prevent problems in writing when there are no reviews
        review = ['']
        
        # variable to count the number of parsed reviews
        num_reviews = 0
        
        for section in soup.find_all(class_="spaceit textReadability word-break pt8 mt8", style="clear: both; border-top: 1px solid #ebebeb;"):
            num_reviews += 1
            
            if num_reviews <= max_reviews:   # we only want the first max_reviews reviews
                
                current_review = section.div.next_sibling.strip()    # first part of the review
                
                # if the first part isn't big enough we go on with the parsing
                if len(current_review) < 500:
                    
                    section = section.div.next_sibling.next_sibling    # next part of the review
                    
                    while section is not None and not len(current_review) > 500:    # we check if the review is finished and if we already took the 500 characters
                        
                        if not isinstance(section, bs4.element.NavigableString) :   # check the type of html element that we just parsed
                            current_review += section.text.strip()
                        else:
                            current_review += section.strip()
                        
                        section = section.next_sibling    # next part of the review
                
                # now we remove tabs and newline characters from the final review
                current_review = current_review.replace('\n', ' ').replace('\t', ' ')
                
                # here we make sure to not cut the 500 characters in the middle of a word
                idx = current_review[:500].rfind(' ')
                if idx<=0: idx = 500
                current_review = current_review[:idx]
                
                review.append(current_review + '\n')
